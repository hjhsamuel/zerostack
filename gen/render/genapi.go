...
func genApi(base *entities.BaseInfo, apiPath, root string) error {
	...
	for _, item := range routeInfos {
		...
		if err = genHandlers(root, item); err != nil {
			return err
		}
		// TODO
	}
...
func genSchemas(root string, info *entities.RouteInfo) error {
	schemaPath := filepath.Join(root, info.Syntax, "schema", info.GroupName+".go")
...
func genHandlers(root string, info *entities.RouteInfo) error {
	...
	handlerPath := filepath.Join(root, info.Syntax, info.GroupName+".go")
	if file.Exists(handlerPath) {
		...
		for _, decl := range f.Decls {
			...
			if _, ok = handlerMap[funcDecl.Name.Name]; !ok {
				continue
			}
			// 检查 handler 是否一致
			if diff, err := diffFunc(funcDecl, handlerMap[funcDecl.Name.Name]); err != nil {
				return err
			} else if diff {
				rewrite = true
			}
			...
			// 移除 handler
			delete(handlerMap, funcDecl.Name.Name)
		}
		if len(handlerMap) != 0 {
			additions, err = GetRenderedContent(info.GroupName, handlerTpl, info.Params)
			if err != nil {
				return err
			}
			rewrite = true
